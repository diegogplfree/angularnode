var app = angular.module('main',[]).controller('mainController',function($scope,$http){
	...
	$scope.baseurl = '/fe/sitesplus/diego/serviciosexternos?debugdiego=truex';        
	...
	$scope.tvlineal = function(){
		$scope.url = $scope.baseurl;
		if($scope.idtvlineal){
			$scope.url += '&idtvlineal='+$scope.idtvlineal;
		}

		if($scope.emailtvlineal){
			$scope.url += '&emailtvlineal='+window.encodeURIComponent($scope.emailtvlineal);
		}
